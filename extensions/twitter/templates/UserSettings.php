...
if ($_POST) {
    ...
    echo '<div class="buddystream_message">
            ' . __('Settings saved', 'buddystream_twitter') . '
        </div>';
    }
    ...
    if (get_user_meta($bp->loggedin_user->id, 'tweetstream_token',1)) {
        ...
        <?php if (! get_site_option('tweetstream_user_settings_syncbp')) {
        ...
        <?php if(get_user_meta($bp->loggedin_user->id, 'tweetstream_token',1)): ?>
            <a href="?network=twitter&reset=true" class="buddystream_reset_button"><?php echo __('Remove Twitter synchronization.','buddystream_facebook');?></a> 
        <?php endif; ?>
        </form>
