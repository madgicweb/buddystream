...
<div class="buddystream_album_navigation_links">
    <ul>
        ...
        //get the active
        $activeExtentions = array();
        foreach(BuddyStreamExtentions::getExtentionsConfigs() as $extention){
            if(get_site_option('buddystream_'.$extention['name'].'_power') == "on" && get_site_option('buddystream_'.$extention['name'].'_album') == "on"){
                echo '<li><a href="?album=' . $extention['name'] . '">'.ucfirst($extention['displayname']).'</a></li>';         
...
$album = "";
if(!isset($_GET['album'])){
    $album = implode(",", $activeExtentions);
}else{
    $album = $_GET['album'];
}
