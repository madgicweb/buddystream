...
if( ! is_admin()){
    wp_register_script('buddystream-colorbox', plugins_url() .
                       '/buddystream/extentions/default/colorbox/jquery.colorbox-min.js', array('jquery'), '1.3.18');
    wp_enqueue_script('buddystream-colorbox');

    wp_register_style('buddystream-colorbox', plugins_url() .
                      '/buddystream/extentions/default/colorbox/colorbox.css', false, '1.3.18', 'screen');
    wp_enqueue_style('buddystream-colorbox');

    if( ! get_site_option('buddystream_nocss')) {
        wp_register_style('buddystream-default', plugins_url() .
                          '/buddystream/extentions/default/style.css', false, '2.5.10', 'screen');
        ...
        if($_POST['topic_title']){
            ...
            foreach(BuddyStreamExtentions::getExtentionsConfigs() as $extention){
                if($extention['hashtag']){
                    if (strpos($_POST['reply_text'], $extention['hashtag'])) {
